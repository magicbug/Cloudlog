...
class Labels extends CI_Controller {
	...
	public function printids() {
		...
		$wwff = $this->input->post('wwff') === "true" ? 1 : 0;
		$this->load->model('labels_model');
		$result = $this->labels_model->export_printrequestedids($ids);

		$this->prepareLabel($result, true, $offset, $grid, $wwff);
	}
	...
	public function print($station_id) {
		...
		$wwff = xss_clean($this->input->post('wwff') ?? 0);
		$this->load->model('stations');
		if ($this->stations->check_station_is_accessible($station_id)) {
			$this->load->model('labels_model');
			$result = $this->labels_model->export_printrequested($clean_id);

			$this->prepareLabel($result, false, $offset, $grid, $wwff);
		} else {
			redirect('labels');
		}
	}

	function prepareLabel($qsos, $jscall = false, $offset = 1, $grid = false, $wwff = false) {
		$this->load->model('labels_model');
		...
		if ($qsos->num_rows() > 0) {
			if ($label->qsos == 1) {
				$this->makeMultiQsoLabel($qsos->result(), $pdf, 1, $offset, $ptype->orientation, $grid, $wwff);
			} else {
				$this->makeMultiQsoLabel($qsos->result(), $pdf, $label->qsos, $offset, $ptype->orientation, $grid, $wwff);
			}
		} else {
	...
	function makeMultiQsoLabel($qsos, $pdf, $numberofqsos, $offset, $orientation, $grid, $wwff) {
		$text = '';
		$current_callsign = '';
		$current_sat = '';
		$current_sat_mode = '';
		...
		foreach($qsos as $qso) {
			...
			( ($qso->COL_BAND_RX !== $current_sat_bandrx) && ($this->pretty_sat_mode($qso->COL_SAT_MODE) !== '')) ) {
			   // ((($qso->COL_SAT_NAME ?? '' !== $current_sat) || ($qso->COL_CALL !== $current_callsign)) && ($qso->COL_SAT_NAME ?? '' !== '') && ($col->COL_BAND_RX ?? '' !== $current_sat_bandrx))) {
				if (!empty($qso_data)) {
					$this->finalizeData($pdf, $current_callsign, $qso_data, $numberofqsos, $orientation, $grid, $wwff);
					$qso_data = [];
				}
	...
	function finalizeData($pdf, $current_callsign, &$preliminaryData, $qso_per_label,$orientation, $grid, $wwff) {
		...
		foreach ($preliminaryData as $key => $row) {
			...
			if($count_qso == $qso_per_label){
				$this->generateLabel($pdf, $current_callsign, $tableData,$count_qso,$qso,$orientation, $grid, $wwff);
	...
	function generateLabel($pdf, $current_callsign, $tableData,$numofqsos,$qso,$orientation,$grid=true,$wwff=true){
		$builder = new \AsciiTable\Builder();
		$builder->addRows($tableData);
			$text = $qso['mycall']." confirming QSO".($numofqsos>1 ? 's' : '')." with ";
		...
		if ($wwff && $qso['mywwff'] != '') { $text .= ", WWFF: ".$qso['mywwff']; }
		$text.="\n";
